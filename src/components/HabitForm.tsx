...
/*
* 1. Create a form component that allows users to:
 *    - input a habit name,
 *    - choose a frequency (daily, weekly, custom days),
 *    - optionally select a color or icon,
 *    - submit the habit to the app state and localStorage.
 * */
...
const HabitForm: React.FC<HabitFormProps> = ({onAddHabit}) => {
    ...
    return ( 
        <form onSubmit={handleSubmit} className="space-y-4 bg-white shadow p-4 rounded-xl">
            <div>
                <label className="block font-semibold text-blue-950 text-sm">Habit Name</label>
                <input 
                type="text"
                value={name}
            ...
            <div>
                <label className="block font-semibold text-blue-950 text-sm">Frequency</label>
                <select
                value={frequency}
            ...
            <div>
                <label className="block font-semibold text-blue-950 text-sm">Color</label>
                <input 
                type="color"
                value={color}
